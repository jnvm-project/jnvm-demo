...
public class AccountManual extends OffHeapObjectHandle {

    private static final long CLASS_ID = OffHeap.Klass.registerUserKlass(AccountManual.class);
    private static final long[] offsets = {0L, 4L};
    private static final long SIZE = Integer.SIZE + Long.SIZE;

    AccountManual(int id, long balance) {
        super();
        setIntegerField(offsets[0], id);
        setLongField(offsets[1], balance);
    }

    public AccountManual(Void v, long offset) {
        super(v, offset);
    }
    ...
    public void transferTo(AccountManual dest, long amount) {
